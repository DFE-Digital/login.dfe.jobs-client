...
describe('when sending roleupdated_v1', () => {
  ...
  it('then it should error if role not passed', async () => {
    await expect(client.notifyRoleUpdated(undefined)).rejects.toThrow('Role must be provided');
  });
  ...
  it('then it should error if role has no id', async () => {
    ...
    await expect(client.notifyRoleUpdated(clonedRole)).rejects.toThrow('Role must have id');
  });
  ...
  it('then it should error if role has no name', async () => {
    ...
    await expect(client.notifyRoleUpdated(clonedRole)).rejects.toThrow('Role must have name');
  });
  ...
  it('then it should error if role has no code', async () => {
    ...
    await expect(client.notifyRoleUpdated(clonedRole)).rejects.toThrow('Role must have code');
  });
});
