...
describe("when sending organisationupdated_v1", () => {
  ...
  it("then it should error if fails to save job", async () => {
    ...
    await expect(
      client.notifyOrganisationUpdated(organisation),
    ).rejects.toThrow("bad times");
    expect(Queue.mock.results[0].value.close.mock.calls.length).toBe(1);
  });

  it("then it should error if organisation not passed", async () => {
    await expect(client.notifyOrganisationUpdated(undefined)).rejects.toThrow(
      "Organisation must be provided",
    );
  });

  it("then it should error if organisation missing id", async () => {
    const brokenOrganisation = Object.assign({}, organisation, {
      id: undefined,
    });

    await expect(
      client.notifyOrganisationUpdated(brokenOrganisation),
    ).rejects.toThrow("Organisation must have id");
  ...
  it("then it should error if organisation missing name", async () => {
    ...
    await expect(
      client.notifyOrganisationUpdated(brokenOrganisation),
    ).rejects.toThrow("Organisation must have name");
    expect(Queue.mock.calls.length).toBe(0);
  });
  ...
  it("then it should error if organisation missing category", async () => {
    ...
    await expect(
      client.notifyOrganisationUpdated(brokenOrganisation),
    ).rejects.toThrow("Organisation must have category");
    expect(Queue.mock.calls.length).toBe(0);
  });
  ...
  it("then it should error if organisation missing category.id", async () => {
    ...
    await expect(
      client.notifyOrganisationUpdated(brokenOrganisation),
    ).rejects.toThrow("Organisation must have category.id");
    expect(Queue.mock.calls.length).toBe(0);
  });
});
